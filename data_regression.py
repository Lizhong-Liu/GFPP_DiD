...
# Health trends of treatment and control group.
health_trend_t = df1[df1['D'] == 1].groupby('YEAR').mean()
for i in dependents:
    health_trend_t['{}'.format(i)].plot()
    plt.show()
health_trend_c = df1[df1['D'] == 0].groupby('YEAR').mean()
for i in dependents:
    health_trend_c['{}'.format(i)].plot()
    plt.show()
